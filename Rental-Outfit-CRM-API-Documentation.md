...
| Param | Type | Description |
|---|---|---|
| `stage` | enum | `Enquiry`, `Confirmed`, `Alteration in Progress`, `Ready for Pickup`, `Picked Up`, `Returned`, `Closed`, `Cancelled` |
...
**Description:** Creates a booking. For each line item, the server performs a **hard availability check** (PRD §7.3.3) before allowing `stage` to be set to `Confirmed` or beyond; if any product conflicts, the request fails with `409` unless the caller has `manager`/`admin` role and passes `"override_conflict": true`.
...
**Auth:** staff+ (most fields) / manager+ (stage → Confirmed with conflict override; pricing overrides beyond max discount)
...
**Description:** Not a hard delete — sets `stage` to `Cancelled`, which releases the product's date range back to available (PRD §9). Use `PATCH .../cancel` (§4.6) in preference; this endpoint is retained for admin cleanup only.
...
**Response `200`:** booking object with `stage: "Cancelled"`. All line-item date ranges are immediately excluded from availability conflict checks.
...
**Description:** Blocked (`409`) if the product has any active (non-Cancelled/Returned) bookings. Recommend using `PATCH` to set `status: "Retired"` instead.
...
**Description:** Checks whether a specific product (or all products in a category) is free for a given date range. This is the endpoint called inline during the New Booking flow (§4.2) before allowing `Confirmed` stage.
...
> Bookings with `stage` in `Cancelled` or `Returned` are excluded from conflict evaluation, per the availability logic in PRD §7.3.3.
...
```json
{
  "success": true,
  "data": {
    "stage_pipeline": ["Enquiry", "Confirmed", "Alteration in Progress", "Ready for Pickup", "Picked Up", "Returned", "Closed", "Cancelled"],
...
| `booking.stage_changed` | Stage transitions (e.g. → `Ready for Pickup`) |
...
```json
{
  ...
  "booking_id": "bkg_9001",
  "data": { "from_stage": "Confirmed", "to_stage": "Ready for Pickup" },
...
## 13. Field-Level Business Rules (Quick Reference)
...
| No overlapping active booking on same `product_code` | `POST /bookings` (stage → Confirmed), `PATCH /bookings/{id}` on date change |
| `discount_percent <= product.max_discount_percent` | `POST/PATCH /bookings` line items |
| Cancelled/Returned bookings excluded from availability conflicts | `GET /availability/check`, `GET /availability/calendar` |
| `product_code` and `receipt_number` unique | `POST /products`, auto-gen on `POST /bookings` |

---

## 14. Open Items for Engineering (carried from PRD)
